    ...
    <v-card>
      ...
      <v-card-text class="lc-body">
        ...
        <v-card
          ...
          variant="outlined"
          class="lc-check mt-2"
          :class="{ 'lc-check--ok': modelImported }"
        >
          <div class="lc-check__head">
            <v-icon size="18">mdi-brain</v-icon>
            <span>{{ t('lineConnect.model') }}</span>
            <v-chip size="x-small" variant="tonal" class="ml-2">
              {{
                modelImported
                  ? t('lineConnect.modelImported')
                  : t('lineConnect.modelBuiltin')
              }}
            </v-chip>
          </div>
          <p class="lc-check__hint">{{ t('lineConnect.modelHint') }}</p>
          ...
          <div class="lc-check__actions">
            ...
            >
              {{
                modelImported
                  ? t('lineConnect.modelReplace')
                  : t('lineConnect.modelPick')
              }}
            </v-btn>
            <v-btn
              v-if="modelImported"
              ...
              @click="onClearModel"
            >
              {{ t('lineConnect.modelClear') }}
            </v-btn>
            <v-chip v-if="modelBusy" size="small" variant="tonal">
              {{ t('lineConnect.modelLoading') }}
            </v-chip>
            <span v-else-if="modelNote" class="lc-check__path">{{ modelNote }}</span>
          </div>
        </v-card>
...
<script setup lang="ts">
  ...
  /* ---------- 识别模型（测试版） ---------- */
  const modelBridge = modelImportBridge()
  const modelSupported = ref(!!modelBridge)
  const modelImported = ref(false)
  const modelName = ref('')
  const modelSizeText = ref('')
  const modelBusy = ref(false)
  const modelNote = ref('')

  function refreshModel() {
    if (!modelBridge) return
    const has = modelBridge.hasModel()
    modelImported.value = has
    modelName.value = has ? modelBridge.modelName() || 'model.onnx' : ''
    if (has) {
      const mb = modelBridge.modelSize() / 1048576
      modelSizeText.value = mb >= 1 ? mb.toFixed(1) + ' MB' : Math.round(mb * 1024) + ' KB'
    } else {
      modelSizeText.value = ''
    }
  }
  ...
  function onPickModel() {
    ...
    modelBridge.pickModel()
    // 用户在系统选择器里直接返回时不会派发事件，这里兜底复位。
    window.setTimeout(() => {
  ...
  function onClearModel() {
    if (!modelBridge) return
    if (!confirm(t('lineConnect.modelClear') + '?')) return
    modelBridge.clearModel()
    // 丢掉缓存的推理会话，否则还会接着用导入的那个模型
    resetSharedSession()
    refreshModel()
    modelNote.value = t('lineConnect.modelReloadHint')
  }

  /** 原生侧选完文件后会派发这个事件。 */
  function onModelImportEvent(event: Event) {
    modelBusy.value = false
    const detail = (event as CustomEvent).detail || {}
    if (!detail.ok) {
      modelNote.value = detail.reason || ''
      return
    }
    // 换模型必须重建会话，否则仍在用旧的图
    resetSharedSession()
  ...
  onMounted(() => {
    ...
    window.addEventListener('model-import', onModelImportEvent)
  ...
  onUnmounted(() => {
    ...
    window.removeEventListener('model-import', onModelImportEvent)
